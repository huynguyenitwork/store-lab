...
### muốn sửa trang login admin các thành phần giao diện input ghi đè|tạo mới
1. truy cập vào trang đăng nhập admin, routing: 
```txt
/admin/login
/api/users/me
/api/payload-preferences/nav
```
2. các file tham khảo
```txt
src\payload.config.ts
src\components\BeforeLogin\index.tsx
```
3. tham khảo sửa đổi các thành phần
* https://github.com/payloadcms/payload/blob/0ceba020/docs/custom-components/custom-views.mdx?plain=1#L22-L47
* https://payloadcms.com/docs/configuration/overview
* https://payloadcms.com/docs/admin/overview#admin-options
* https://deepwiki.com/payloadcms/payload/2-configuration-system
### muốn tạo url router + view component để hiểu cách tạo và hoạt động
1. tham khảo nguồn
* https://deepwiki.com/payloadcms/payload/4.4-custom-components-and-server-functions
2. tạo file view component
```txt
src\components\MyCustomView\index.tsx
```
3. cấu hình đường dẫn url trỏ đến component `src\payload.config.ts`
```ts
views: {
   myCustomView: {
...
```
4. chạy lại lệnh khởi tạo import map và kiểm tra file `importMap.js`
```bash
